...
# --- ASSESSMENT CLASS TAXONOMY ---
# 6 classes derived from catalog `keys` analysis, used for class-based retrieval boosting
ASSESSMENT_CLASSES = {
    "cognitive_aptitude": {
        "triggers": ["cognitive", "aptitude", "reasoning", "verify", "numerical", "deductive",
                     "inductive", "verbal", "g+", "iq", "intelligence", "mental ability",
                     "problem solving", "analytical", "ability test"],
        "catalog_keys": ["Ability & Aptitude"],
        "default_slugs": ["shl-verify-interactive-g"],
        "boost_weight": 0.15,
        "retrieval_query": "verify cognitive ability reasoning aptitude test",
    },
    "knowledge_skills": {
        "triggers": ["knowledge", "technical", "programming", "engineering", "multi-choice",
                     "certification", "domain test", "skill test", "technology"],
        "catalog_keys": ["Knowledge & Skills"],
        "default_slugs": [],  # Too many — rely on skill-to-slug mapping
        "boost_weight": 0.08,
        "retrieval_query": "knowledge skills technical multi-choice test",
    },
    "personality_behavioral": {
        "triggers": ["personality", "behavioral", "behaviour", "opq", "motivation",
                     "workplace style", "culture fit", "work style", "temperament",
                     "traits", "character", "interpersonal"],
        "catalog_keys": ["Personality & Behavior"],
        "default_slugs": ["occupational-personality-questionnaire-opq32r"],
        "boost_weight": 0.15,
        "retrieval_query": "OPQ personality questionnaire workplace behavior motivation",
    },
    "situational_judgment": {
        "triggers": ["sjt", "situational", "judgment", "judgement", "scenarios",
                     "decision making", "managerial judgment", "workplace decisions"],
        "catalog_keys": ["Biodata & Situational Judgment"],
        "default_slugs": [],  # Determined by seniority
        "boost_weight": 0.12,
        "retrieval_query": "situational judgment scenarios decision making effectiveness",
    },
    "simulations_interactive": {
        "triggers": ["simulation", "simulated", "coding", "automata", "data entry",
                     "typing", "svar", "spoken", "contact center", "contact centre",
                     "call center", "call centre", "chat simulation", "live coding",
                     "hands-on", "practical", "interactive exercise"],
        "catalog_keys": ["Simulations", "Assessment Exercises"],
        "default_slugs": [],
        "boost_weight": 0.12,
        "retrieval_query": "simulation interactive exercise coding data entry spoken",
    },
    "competency_360_development": {
        "triggers": ["competency", "ucf", "360", "feedback", "hipo", "high potential",
                     "development", "reskill", "re-skill", "talent audit", "pjm",
                     "person-job match", "profiler", "leadership development",
                     "restructuring", "upskill"],
        "catalog_keys": ["Competencies", "Development & 360"],
        "default_slugs": [],
        "boost_weight": 0.12,
        "retrieval_query": "competency framework 360 feedback development potential",
    },
}

def detect_active_classes(user_text: str, constraint_state: dict) -> dict:
    """Scan user text + constraints to determine which assessment classes are relevant.
    
    Returns dict mapping class_name -> activation_score (0.0 to 1.0).
    Only classes with score > 0 are considered active.
    """
    text_lower = user_text.lower()
    skills = [s.lower() for s in (constraint_state.get("skills") or [])]
    labels = [l.lower() for l in (constraint_state.get("labels") or [])]
    seniority = (constraint_state.get("seniority") or "").lower()
    role = (constraint_state.get("role") or "").lower()
    
    active = {}
    
    for cls_name, cls_data in ASSESSMENT_CLASSES.items():
        score = 0.0
        
        # Check trigger keywords against user text
        for trigger in cls_data["triggers"]:
            if trigger in text_lower:
                score += 0.4
                break  # One hit is enough for text-based activation
        
        # Check labels from constraint state (e.g., user asked for "cognitive")
        for label in labels:
            for trigger in cls_data["triggers"]:
                if trigger in label or label in trigger:
                    score += 0.5
                    break
        
        # Implicit activation based on context
        # Hiring roles almost always need personality + cognitive
        if cls_name == "personality_behavioral":
            if any(kw in text_lower for kw in ["hiring", "selection", "recruit", "screening"]):
                score += 0.3
        if cls_name == "cognitive_aptitude":
            if any(kw in text_lower for kw in ["hiring", "selection", "recruit"]):
                score += 0.2
        
        # Graduate/entry → SJT
        if cls_name == "situational_judgment":
            if seniority in ["graduate", "entry-level", "entry level"]:
                score += 0.3
        
        # Development/reskill → competency/360
        if cls_name == "competency_360_development":
            if any(kw in text_lower for kw in ["development", "reskill", "re-skill", "audit", "training"]):
                score += 0.4
        
        # Contact centre / customer service → simulations
        if cls_name == "simulations_interactive":
            if any(kw in text_lower for kw in ["contact cent", "call cent", "customer service", "agent"]):
                score += 0.3
        
        # Named technical skills → knowledge tests
        if cls_name == "knowledge_skills":
            if skills:
                score += 0.3
        
        if score > 0:
            active[cls_name] = min(score, 1.0)
...
def node_c_role_classifier(state: GraphState):
    ...
    # Detect active assessment classes to guide query generation
    active_classes = detect_active_classes(all_user_msgs, c_state)
    active_class_names = list(active_classes.keys())
...
Active assessment classes detected: {json.dumps(active_class_names)}

Generate 4-6 specific, diverse search queries that will help find ALL relevant assessments.

IMPORTANT: Generate at least ONE query for EACH active assessment class listed above. The 6 possible classes are:
- cognitive_aptitude → query for: "verify cognitive ability reasoning aptitude test"
- knowledge_skills → query for specific technologies/domains mentioned by user
- personality_behavioral → query for: "OPQ personality questionnaire workplace behavior"
- situational_judgment → query for: "scenarios situational judgment decision making"
- simulations_interactive → query for: "simulation coding data entry spoken language contact center"
- competency_360_development → query for: "competency framework 360 feedback development HiPo"

Also think about:
1. Role-specific knowledge tests (e.g., "Java programming assessment")
2. Industry/domain-specific assessments (e.g., "safety dependability manufacturing")
...
def node_e_retriever(state: GraphState):
    ...
    # --- CLASS-BASED RETRIEVAL (new 3rd channel) ---
    active_classes = detect_active_classes(all_user_text, c_state)
    ...
    # 2. Add raw user message as a query
    ...
    # 5. CLASS-AWARE queries — one per active class
    for cls_name, cls_score in active_classes.items():
        cls_data = ASSESSMENT_CLASSES[cls_name]
        queries.append(cls_data["retrieval_query"])
    ...
    # 6. Direct keyword matching on catalog
    ...
    # 7. CLASS-BASED CATALOG BOOSTING — boost items whose keys match active classes
    for cls_name, cls_score in active_classes.items():
        cls_data = ASSESSMENT_CLASSES[cls_name]
        boost = cls_data["boost_weight"] * cls_score
        for slug, item in catalog_dict.items():
            item_keys = item.get("keys", [])
            if any(ck in item_keys for ck in cls_data["catalog_keys"]):
                all_results[slug] = all_results.get(slug, 0) + boost
            
    top_slugs = sorted(all_results.keys(), key=lambda k: all_results[k], reverse=True)[:18]
    ...
    # CLASS-BASED default slug injection
    for cls_name, cls_score in active_classes.items():
        for slug in ASSESSMENT_CLASSES[cls_name]["default_slugs"]:
            injected.add(slug)
    ...
    print(f"Retrieved {len(final_results)} results (injected {len(injected)} canonical, {len(active_classes)} classes active).")
...
def node_f_synthesizer(state: GraphState):
    ...
    # Detect active assessment classes for context
    all_msgs_text = " ".join([m["content"] for m in state["messages"] if m["role"] == "user"])
    active_classes = detect_active_classes(all_msgs_text, c_state)
    active_class_summary = ", ".join(active_classes.keys()) if active_classes else "none detected yet"
    ...
    # Determine what info is missing — prioritize by importance
    missing_dims = []
    all_msgs_lower = all_msgs_text.lower()
    
    # Purpose is the MOST important missing info
    purpose_clear = any(kw in all_msgs_lower for kw in ["hiring", "selection", "screening", "recruit", "development", "reskill", "re-skill", "audit", "training"])
    ...
    if not c_state.get("skills") and not c_state.get("labels"): missing_dims.append("skills_or_assessment_type")
...
DETECTED ASSESSMENT CLASSES: {active_class_summary}
...
If VAGUE and not FORCE:
  Ask ONE targeted clarifying question about the MOST CRITICAL missing info from: {missing_info}.
  Priority order for questions:
  1. PURPOSE first: "Is this for hiring/selection, or for development/re-skilling? This changes which products I recommend."
  2. ROLE second: "What specific role are you hiring for? E.g., software engineer, financial analyst, sales rep, contact centre agent..."
  3. SENIORITY third: "What seniority level? Graduate/entry-level gets Graduate Scenarios; senior gets advanced tests; executive gets leadership reports."
  4. SPECIFICS last: Ask about the specific dimension that matters most for the detected classes:
     - If contact centre detected → ask about language/accent variant (US, UK, AUS, Indian)
     - If tech role detected → ask about backend vs frontend weighting
     - If leadership detected → ask about selection vs development purpose
  Use domain expertise to guide the user — mention specific SHL products that depend on their answer.
  Return EMPTY recommendations. Set end_of_conversation=false.
  
  CRITICAL: Do NOT generate recommendations when info is missing. Gather info FIRST.
...
If CONSTRAINED/REFINE/FORCE:
  ...
  Set end_of_conversation=true (unless the user is mid-refinement and might want to adjust).
...
def build_graph():
    ...
    def router_b(state):
        intent = state.get("intent", "VAGUE")
        if intent == "VAGUE":
            # VAGUE: skip heavy retrieval, just extract state and ask clarifying question
            return "node_d"
        elif intent == "OUT_OF_SCOPE":
            return "node_f"
        elif intent == "COMPARE":
            return "node_e"
        else: # CONSTRAINED, REFINE — run full pipeline
            return "node_c"
    ...
    # After state extraction, route based on intent
    def router_d(state):
        intent = state.get("intent", "VAGUE")
        if intent == "VAGUE":
            # Skip retrieval for VAGUE — go directly to synthesizer for clarifying question
            return "node_f"
        else:
            return "node_e"
    
    workflow.add_conditional_edges("node_d", router_d)
...
def chat(request: ChatRequest):
    ...
    # Persist constraint_state across turns
    incoming_state = request.constraint_state or {}
    
    initial_state = {
        "messages": msgs,
        "constraint_state": incoming_state,
        "classified_roles": [],
        "retrieval_queries": [],
        "retrieval_results": [],
        "compare_slugs": []
    ...
    final = result.get("final_response", {"reply": "Error", "recommendations": [], "end_of_conversation": False})
    
    # Attach accumulated constraint_state to response for round-tripping
    if isinstance(final, dict):
        final["constraint_state"] = result.get("constraint_state", incoming_state)
